...
func (cu *CategoryUpdate) AddTask(t ...*Task) *CategoryUpdate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
...
func (cu *CategoryUpdate) ClearTask() *CategoryUpdate {
...
func (cu *CategoryUpdate) RemoveTask(t ...*Task) *CategoryUpdate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
...
func (cu *CategoryUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if cu.mutation.TaskCleared() {
		edge := &sqlgraph.EdgeSpec{
			...
			Columns: []string{category.TaskColumn},
			Bidi:    false,
...
// AddTask adds the "task" edges to the Task entity.
func (cuo *CategoryUpdateOne) AddTask(t ...*Task) *CategoryUpdateOne {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
...
// ClearTask clears all "task" edges to the Task entity.
func (cuo *CategoryUpdateOne) ClearTask() *CategoryUpdateOne {
...
// RemoveTask removes "task" edges to Task entities.
func (cuo *CategoryUpdateOne) RemoveTask(t ...*Task) *CategoryUpdateOne {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
