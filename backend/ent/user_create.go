...
func (uc *UserCreate) AddTask(t ...*Task) *UserCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
...
func (uc *UserCreate) AddReport(r ...*Report) *UserCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
...
func (uc *UserCreate) AddSettings(s ...*Settings) *UserCreate {
	ids := make([]int, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
...
func (uc *UserCreate) AddSharingIDs(ids ...int) *UserCreate {
	uc.mutation.AddSharingIDs(ids...)
	return uc
}

// AddSharing adds the "sharing" edges to the Sharing entity.
func (uc *UserCreate) AddSharing(s ...*Sharing) *UserCreate {
	ids := make([]int, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return uc.AddSharingIDs(ids...)
}
...
func (uc *UserCreate) AddProject(p ...*Project) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.TaskIDs(); len(nodes) > 0 {
	...
	if nodes := uc.mutation.ReportIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := uc.mutation.SharingIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
