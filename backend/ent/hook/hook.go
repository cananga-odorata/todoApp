...
// The SharingFunc type is an adapter to allow the use of ordinary
// function as Sharing mutator.
type SharingFunc func(context.Context, *ent.SharingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SharingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SharingMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SharingMutation", m)
}
