...
func (rq *ReportQuery) QueryUserOwner() *UserQuery {
...
func (rq *ReportQuery) Clone() *ReportQuery {
	...
	return &ReportQuery{
		...
		withUserOwner: rq.withUserOwner.Clone(),
...
func (rq *ReportQuery) WithUserOwner(opts ...func(*UserQuery)) *ReportQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withUserOwner = query
	return rq
}
...
func (rq *ReportQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Report, error) {
	var (
		...
		_spec       = rq.querySpec()
		loadedTypes = [1]bool{
			rq.withUserOwner != nil,
		}
	)
	if rq.withUserOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withUserOwner; query != nil {
		if err := rq.loadUserOwner(ctx, query, nodes, nil,
			func(n *Report, e *User) { n.Edges.UserOwner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReportQuery) loadUserOwner(ctx context.Context, query *UserQuery, nodes []*Report, init func(*Report), assign func(*Report, *User)) error {
