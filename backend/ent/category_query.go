...
func (cq *CategoryQuery) QueryTask() *TaskQuery {
...
func (cq *CategoryQuery) WithTask(opts ...func(*TaskQuery)) *CategoryQuery {
	query := (&TaskClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Category, init func(*Category), assign func(*Category, *Task)) error {
	...
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.TaskColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n._Category_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "_Category_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "_Category_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
