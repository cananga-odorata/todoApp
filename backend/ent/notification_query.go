...
func (nq *NotificationQuery) QueryUserOwner() *UserQuery {
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withUserOwner: nq.withUserOwner.Clone(),
...
func (nq *NotificationQuery) WithUserOwner(opts ...func(*UserQuery)) *NotificationQuery {
	query := (&UserClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withUserOwner = query
	return nq
}
...
//		GroupBy(notification.FieldTaskID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Notification.Query().
//		Select(notification.FieldTaskID).
//		Scan(ctx, &v)
...
func (nq *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	var (
		...
		_spec       = nq.querySpec()
		loadedTypes = [1]bool{
			nq.withUserOwner != nil,
		}
	)
	if nq.withUserOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withUserOwner; query != nil {
		if err := nq.loadUserOwner(ctx, query, nodes, nil,
			func(n *Notification, e *User) { n.Edges.UserOwner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NotificationQuery) loadUserOwner(ctx context.Context, query *UserQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *User)) error {
