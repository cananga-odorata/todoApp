...
func (cq *CommentQuery) QueryUserOwner() *UserQuery {
...
// QueryTaskOwner chains the current query on the "taskOwner" edge.
func (cq *CommentQuery) QueryTaskOwner() *TaskQuery {
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withUserOwner: cq.withUserOwner.Clone(),
		withTaskOwner: cq.withTaskOwner.Clone(),
...
func (cq *CommentQuery) WithUserOwner(opts ...func(*UserQuery)) *CommentQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withUserOwner = query
	return cq
}

// WithTaskOwner tells the query-builder to eager-load the nodes that are connected to
// the "taskOwner" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommentQuery) WithTaskOwner(opts ...func(*TaskQuery)) *CommentQuery {
	query := (&TaskClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withTaskOwner = query
	return cq
}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	var (
		...
		_spec       = cq.querySpec()
		loadedTypes = [2]bool{
			cq.withUserOwner != nil,
			cq.withTaskOwner != nil,
		}
	)
	if cq.withUserOwner != nil || cq.withTaskOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withUserOwner; query != nil {
		if err := cq.loadUserOwner(ctx, query, nodes, nil,
			func(n *Comment, e *User) { n.Edges.UserOwner = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withTaskOwner; query != nil {
		if err := cq.loadTaskOwner(ctx, query, nodes, nil,
			func(n *Comment, e *Task) { n.Edges.TaskOwner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadUserOwner(ctx context.Context, query *UserQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *User)) error {
...
func (cq *CommentQuery) loadTaskOwner(ctx context.Context, query *TaskQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Task)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i]._Task_id == nil {
			continue
		}
		fk := *nodes[i]._Task_id
		if _, ok := nodeids[fk]; !ok {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "_Task_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
