...
func (aq *AuthenticationQuery) QueryUserOwner() *UserQuery {
...
func (aq *AuthenticationQuery) Clone() *AuthenticationQuery {
	...
	return &AuthenticationQuery{
		...
		withUserOwner: aq.withUserOwner.Clone(),
...
func (aq *AuthenticationQuery) WithUserOwner(opts ...func(*UserQuery)) *AuthenticationQuery {
	query := (&UserClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withUserOwner = query
	return aq
}
...
//		GroupBy(authentication.FieldSessionID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Authentication.Query().
//		Select(authentication.FieldSessionID).
//		Scan(ctx, &v)
...
func (aq *AuthenticationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Authentication, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [1]bool{
			aq.withUserOwner != nil,
		}
	)
	if aq.withUserOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withUserOwner; query != nil {
		if err := aq.loadUserOwner(ctx, query, nodes, nil,
			func(n *Authentication, e *User) { n.Edges.UserOwner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthenticationQuery) loadUserOwner(ctx context.Context, query *UserQuery, nodes []*Authentication, init func(*Authentication), assign func(*Authentication, *User)) error {
