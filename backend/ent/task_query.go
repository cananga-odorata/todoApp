...
type TaskQuery struct {
	...
	withProjectOwner  *ProjectQuery
	withComment       *CommentQuery
	withCategoryOwner *CategoryQuery
	withFKs           bool
...
func (tq *TaskQuery) QueryUserOwner() *UserQuery {
...
// QueryProjectOwner chains the current query on the "projectOwner" edge.
func (tq *TaskQuery) QueryProjectOwner() *ProjectQuery {
...
func (tq *TaskQuery) QueryComment() *CommentQuery {
...
// QueryCategoryOwner chains the current query on the "categoryOwner" edge.
func (tq *TaskQuery) QueryCategoryOwner() *CategoryQuery {
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withUserOwner:     tq.withUserOwner.Clone(),
		withProjectOwner:  tq.withProjectOwner.Clone(),
		withComment:       tq.withComment.Clone(),
		withCategoryOwner: tq.withCategoryOwner.Clone(),
...
func (tq *TaskQuery) WithUserOwner(opts ...func(*UserQuery)) *TaskQuery {
	query := (&UserClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withUserOwner = query
	return tq
}

// WithProjectOwner tells the query-builder to eager-load the nodes that are connected to
// the "projectOwner" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithProjectOwner(opts ...func(*ProjectQuery)) *TaskQuery {
	query := (&ProjectClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withProjectOwner = query
	return tq
}
...
func (tq *TaskQuery) WithComment(opts ...func(*CommentQuery)) *TaskQuery {
	query := (&CommentClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withComment = query
	return tq
}

// WithCategoryOwner tells the query-builder to eager-load the nodes that are connected to
// the "categoryOwner" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithCategoryOwner(opts ...func(*CategoryQuery)) *TaskQuery {
	query := (&CategoryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withCategoryOwner = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [4]bool{
			tq.withUserOwner != nil,
			tq.withProjectOwner != nil,
			tq.withComment != nil,
			tq.withCategoryOwner != nil,
		}
	)
	if tq.withUserOwner != nil || tq.withProjectOwner != nil || tq.withCategoryOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withUserOwner; query != nil {
		if err := tq.loadUserOwner(ctx, query, nodes, nil,
			func(n *Task, e *User) { n.Edges.UserOwner = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withProjectOwner; query != nil {
		if err := tq.loadProjectOwner(ctx, query, nodes, nil,
			func(n *Task, e *Project) { n.Edges.ProjectOwner = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withComment; query != nil {
		if err := tq.loadComment(ctx, query, nodes,
			func(n *Task) { n.Edges.Comment = []*Comment{} },
			func(n *Task, e *Comment) { n.Edges.Comment = append(n.Edges.Comment, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withCategoryOwner; query != nil {
		if err := tq.loadCategoryOwner(ctx, query, nodes, nil,
			func(n *Task, e *Category) { n.Edges.CategoryOwner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadUserOwner(ctx context.Context, query *UserQuery, nodes []*Task, init func(*Task), assign func(*Task, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i]._User_id == nil {
			continue
		}
		fk := *nodes[i]._User_id
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "_User_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadProjectOwner(ctx context.Context, query *ProjectQuery, nodes []*Task, init func(*Task), assign func(*Task, *Project)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i]._Project_id == nil {
			continue
		}
		fk := *nodes[i]._Project_id
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "_Project_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadComment(ctx context.Context, query *CommentQuery, nodes []*Task, init func(*Task), assign func(*Task, *Comment)) error {
	...
	query.Where(predicate.Comment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.CommentColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n._Task_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "_Task_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "_Task_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TaskQuery) loadCategoryOwner(ctx context.Context, query *CategoryQuery, nodes []*Task, init func(*Task), assign func(*Task, *Category)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i]._Category_id == nil {
			continue
		}
		fk := *nodes[i]._Category_id
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "_Category_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
