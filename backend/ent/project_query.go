...
func (pq *ProjectQuery) QueryUserOwner() *UserQuery {
...
func (pq *ProjectQuery) QueryTask() *TaskQuery {
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withUserOwner: pq.withUserOwner.Clone(),
...
func (pq *ProjectQuery) WithUserOwner(opts ...func(*UserQuery)) *ProjectQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withUserOwner = query
	return pq
}

// WithTask tells the query-builder to eager-load the nodes that are connected to
// the "task" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProjectQuery) WithTask(opts ...func(*TaskQuery)) *ProjectQuery {
	query := (&TaskClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withUserOwner; query != nil {
		if err := pq.loadUserOwner(ctx, query, nodes,
			func(n *Project) { n.Edges.UserOwner = []*User{} },
			func(n *Project, e *User) { n.Edges.UserOwner = append(n.Edges.UserOwner, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withTask; query != nil {
		if err := pq.loadTask(ctx, query, nodes,
			func(n *Project) { n.Edges.Task = []*Task{} },
			func(n *Project, e *Task) { n.Edges.Task = append(n.Edges.Task, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadUserOwner(ctx context.Context, query *UserQuery, nodes []*Project, init func(*Project), assign func(*Project, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(project.UserOwnerTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(project.UserOwnerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(project.UserOwnerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(project.UserOwnerPrimaryKey[1]))
...
func (pq *ProjectQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Project, init func(*Project), assign func(*Project, *Task)) error {
	...
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.TaskColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n._Project_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "_Project_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "_Project_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
